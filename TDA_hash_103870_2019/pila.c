...
void* pila_tope(pila_t* pila){
  if(pila == NULL || pila_vacia(pila)){
    return NULL;//chequear(tambien devuelve null si la pila es null)
  }
  //aca hace falta fijarse si la pila es null? porque ya loc hequie en la funcion en la cual la llamo
  //have falta chequear que no este vacia?
...
int pila_cantidad(pila_t* pila){
  if(pila == NULL){
    return VACIA;//aca devuelvo error
  }
  return pila->tope;
}
